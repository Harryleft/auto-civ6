...
#: Civ6 特有红线（审查 R10 之后新增）：军力代差硬打、攻城手段不足、防御不足开战。
#:
#: 实测校准：真机 10 回合跑里这些红线在**明显不适用**的候选（建城、选研究、
#: 排生产）上仍给出 0.43–0.55，恰好跨过 0.50；而 0.50 门槛下每条决定都可能被
#: 随机拦下。红线承担"阻止明显错误"的职责，因此要求**明确踩线**才拦：阈值提到
#: 0.75，并把问句改成"是否确实是一个 X 动作并且缺少 Y"的适用性写法（不适用时
#: 应接近 0）。两处一起改，避免用单点概率噪声决定整局。
CIV6_REDLINE_THRESHOLD = 0.75
...
REVIEW_QUESTIONS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "role": ROLE_BLOCKING,
        "blocking_when": "true",
        "instructions": "候选是否是一个**单位对单位的军事攻击**，并且要用明显低一级的兵种硬打高一级兵种（wiki：差 10 点战力伤害显著拉开，差 36+ 必被秒杀）？",
        "criteria": {
            "true": "候选确实是单位攻击，且存在代差硬打、没有夹击/地形/驻防补偿。",
            "false": "候选**不是**单位攻击（例如建城、选研究/市政、排生产、移动、改良），或已有足够补偿。",
        },
    },
    {
        ...
        "role": ROLE_BLOCKING,
        "blocking_when": "true",
        "instructions": "候选是否是一个**军事攻城动作**（攻击或试图夺取一座城市），并且缺少必要手段（wiki：远程只打兵不清城防，破墙必须靠攻城单位或破城锤，最后需近战占领）？",
        "criteria": {
            "true": "候选确实是攻城/夺城动作，且没有攻城单位或破城锤，或没有近战单位收尾。",
            "false": "候选**不是**军事攻城动作（例如建城、选研究/市政、排生产、移动、改良），或攻城手段齐备。",
        },
    },
    {
        "id": "unprepared_war",
        ...
        "role": ROLE_BLOCKING,
        "blocking_when": "true",
        "instructions": "候选是否在防御明显不足的情况下**主动开启或扩大战争**？",
        "criteria": {
            "true": "候选包含宣战或扩大战线的动作，且我方在无城墙/无驻军/无盟友的状态下。",
            "false": "候选**不包含**宣战或扩大战线（例如建城、选研究/市政、排生产、移动、改良），或防御已就绪。",
        },
    },
    {
        "id": "action_cost",
