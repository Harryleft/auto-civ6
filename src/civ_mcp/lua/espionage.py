...
def build_spy_mission(
    ...
    return " ".join(
        [
            ...
            "local opAfter = nil",
            "local ok_op, opIdx = pcall(function() return unit:GetSpyOperation() end)",
            "if ok_op and opIdx and opIdx >= 0 then",
            "  for row in GameInfo.UnitOperations() do",
            "    if row.Index == opIdx then opAfter = row.OperationType end",
            "  end",
            "end",
            f'if opAfter and opAfter ~= "none" then',
            f'  print("OK:SPY_MISSION|{mission_type} mission launched at ({target_x},{target_y}). (verified: " .. opAfter .. ")")',
            "else",
            f'  print("OK:SPY_MISSION_SUBMITTED|{mission_type} mission requested at ({target_x},{target_y}) but not yet reflected — re-check with get_spies()")',
            "end",
            f'print("{sentinel}")',
        ]
    )
