...
未知的未知不可能靠继续增加节点和规则消灭。正确做法是让系统在基本假设错误时也能停下来，而不是自信地做错事。

| 致命假设 | 真实盲点 | 最小验证 |
|---|---|---|
| 图拿到的数据足够真实 | 游戏只暴露部分可见世界；当前 `get_threat_scan()` 甚至还没有进入 governance snapshot | 让敌军“出现 → 进入迷雾 → 再出现”，中间必须是“未知”，不能变成“不存在” |
| 同一个 ID 永远代表同一个对象 | 城市易主、单位死亡重建、读档和新游戏可能复用 ID；当前 reload 也未证明所有旧 Belief/Goal 都不会泄漏到新分支 | 做一次城市易主、单位重建和读档，确认关系接续正确，旧分支内容只在历史中出现 |
| 有了图，决策自然会更好 | 图首先提升的是可追溯性，不一定提升玩法；错误数据会产生更有说服力的错误 | 只做一个影子切片，与旧路径比较错误动作、重复查询和人工纠正次数；没有改善就停止扩张 |
| 有完整链路，人就能理解 | 节点和 ID 对人没有意义；用户看不懂，就无法发现错误或安全接管 | 每个动作必须用五句话说明：看见什么、判断什么、为何选择、执行什么、如何验证 |

整个方案的依赖链其实只有一条：

> 输入可信 → 身份与时间正确 → 决策可解释 → 动作可验证。

最弱的一环是“输入代表什么”，不是图存在哪里。因此第一阶段只做影子图，不替换现有路径；出现以下任一情况就停止图化并先修地基：

1. 真实威胁数据无法进入同回合 snapshot。
2. “未观察”仍会被当成“已删除”。
3. 读档后旧分支的事实或计划仍影响当前决策。
4. 同一输入经过旧路径和图路径得到不同关键结论。
5. 一个动作无法生成上述五句话，或 read-back 无法判断它是否生效。

这五条全部通过后，才迁移第一个 Department。E.9 是实现参考，使用者不需要理解其中的类名和消息协议。
