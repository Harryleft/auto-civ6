...
Each entry includes: tool name, category (query/action/turn), input parameters, full result (信封 JSON 或文本), success/failure, wall-clock duration, turn number, and session ID.
...
Typical size: 10–50 MB per full game (the `result` field stores the complete 信封 JSON, sized by query data).
