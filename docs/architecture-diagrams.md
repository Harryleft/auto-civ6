...
`end_turn` polls for a bounded window per call — measured turns are median 15.5s, p90 26.3s, p97.6 90s across 710 recorded calls — so one call's declared ceiling is ~173s (poll 102s + the bounded congress/diplomacy re-checks + a 25s query reserve). Waiting beyond that continues across calls: a still-processing turn returns `TURN_PENDING`, and calling `end_turn` again keeps polling **without** re-sending `ACTION_ENDTURN`. A turn is only called a hang after 600s of cumulative waiting on the same pending turn.

**A game relaunch is not part of that loop.** `restart_and_load` / `launch_game` / `load_game_save`'s FrontEnd path wait on the game's own launch (up to 60s for the process, 180s for FireTuner's port) and are operator steps, so they are excluded from the loop's host deadline; `civ_mcp.end_turn.restart_and_load_budget_seconds()` declares their ceiling separately. `end_turn` never relaunches — that is what used to force a ~50-minute deadline for an operation whose normal case is seconds, and it also hid whether a turn had merely been slow or had been wedged and restarted.

A congress turn is not inherently long: the congress opens inside `ACTION_ENDTURN`, and *waiting passively* for its screen is what used to cost 10-20 minutes. The wait loop now drives the session itself (vote the live resolutions, submit) from t+5s, so those turns normally resolve in seconds. An undriven congress is reported as `CONGRESS_NOT_DRIVEN`, **never** as a hang — the end-turn blocker query reports nothing while the congress segment runs, so misclassifying it used to kill and reload a healthy game over a missing vote.
