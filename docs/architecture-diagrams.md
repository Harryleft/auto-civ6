...
`end_turn` is the outlier by two orders of magnitude: its polling ladder runs up to ~830s on a congress turn (550s plain + 180s congress slack) plus a bounded number of congress probes, so the declared ceiling for one call is ~1130s including the query reserve. **Recovery is a separate call** (kill, relaunch, reload ≈ 375s ceiling) and is not summed with it: the client timeout applies per call, so folding three relaunches into `end_turn` is what used to force a ~50-minute deadline for an operation whose normal case is seconds, and it also hid whether a turn had merely been slow or had been wedged and restarted. The ceiling is derived in `civ_mcp.end_turn` (`longest_mcp_call_seconds()`) and the client timeout in `integrations/deepseek-harness/civ6.cordis.yml` must exceed it; `tests/test_end_turn_budget.py` enforces that. Budget client timeouts accordingly.
