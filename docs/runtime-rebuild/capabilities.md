...
- `get_runtime_context` 未包含且未由本清单逐项声明的细粒度读取，例如通用地图/视野、战斗或
  路径估算、外交交易选项、单位/宗教详情。`get_wonder_placements` 仅是已声明的奇观格点读取例外。
- 尚未暴露的写操作：建设者的修复、清除和筑路，非唯一/条款不完整交易的接受或拒绝、宗教创立/信条选择/传播/增强（万神殿除外）、间谍、世界议会投票、奇观生产及
  任何未列在上表的 mutation。
