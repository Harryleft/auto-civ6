...
| 常规 mutation | `move_unit`、`set_city_production`、`set_research`、`set_civic`、`found_city`、`choose_pantheon`、`choose_dedication`、`recruit_great_person` | 每次请求绑定 game/branch/operation/decision turn，经 `SessionKernel` 单次发送，且仅由领域 readback 确认。 |
...
- 尚未暴露的写操作：单位/城市攻击、升级/晋升、购买、建设者、商路、交易提议或
