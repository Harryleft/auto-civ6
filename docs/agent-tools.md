...
从 t+5s 起按 15 秒一轮的密集节奏尝试，之后按 60 秒一轮稀疏覆盖整个开会窗口，
每次都会读取**当时真实**的决议清单、套用已注册的票型（未注册则每项投 1 张免费票）、
投票并提交——等同于人打开议会、选边、点投票。所以：
...
- **没驱动成的议会回合不会被当成 AI 挂起，也绝不会因此重启游戏。** 议会界面挂着时
  ...
  杀掉并重启一个健康的游戏最多三次。现在这种情况返回 `CONGRESS_NOT_DRIVEN`
  （带 `CONGRESS_NOT_DRIVEN_IS_NOT_A_HANG`），要求重新注册票型后重试，禁止重启。
- 只有真正卡死的 AI 回合才返回 `HANG`，而且 **`end_turn` 自己不重启游戏**：
  恢复是独立一步（`restart_and_load` → `get_game_overview` 核对），见
