...
**不要靠等待过议会回合。** 议会在 `ACTION_ENDTURN` 内开会并停在议会界面上，
被动等它结束正是议会回合耗时 10–20 分钟的原因。等待循环现在会自己驱动议会：
从 t+5s 起每 15 秒一轮尝试 5 次，之后每 60 秒一轮再试 5 次，覆盖整个开会窗口
（按**累计等待**计算，所以跨多次 `end_turn` 继续有效）。每次都会读取**当时真实**的
决议清单、套用已注册的票型（未注册则每项投 1 张免费票）、投票并提交——等同于人打开
议会、选边、点投票。所以：

- 已注册票型时议会回合通常在数十秒内推进，不会走满等待预算；
- 没注册票型也不会卡住：驱动器默认每项投 1 张免费票，不消耗支持度；
- 盲关议会界面（`dismiss_popup`）仍是独立且极小的兜底（最多 2 次，60s 起），
  因为在 AI 处理期间反复关 UI 是历史卡死的成因；
- **没驱动成的议会回合不会被当成 AI 挂起，也绝不会因此重启游戏。** 议会界面挂着时
  回合号不动、阻塞项查询也返回空，过去的判定因此把「没人投票」误判成「AI 卡死」，
  杀掉并重启一个健康的游戏最多三次。现在返回 `CONGRESS_NOT_DRIVEN`
  （带 `CONGRESS_NOT_DRIVEN_IS_NOT_A_HANG`），要求重新注册票型后重试，禁止重启。
- 回合仍在正常处理时返回 `TURN_PENDING`：再次调用 `end_turn` 继续等即可，
  重复调用不会重发结束回合请求。只有累计等待超过 600 秒才判 `HANG`，
  而重启属于循环之外的恢复动作，见 [游戏恢复](agent-recovery.md)。

`hash` 只在会话内部可信：开会前的预览列的是上一届决议，因此持久可用的键是决议
**类型名**（`type`），驱动器会拿它在真实清单里反查匹配。
