...
首版只覆盖仓库实际测试的组合：**苏美尔 / 吉尔伽美什 × `RULESET_EXPANSION_2`**
（`docs/paper/scenario-spec.md` 的 Scenario C）。事实取自仓库内的场景规格。

- 其它文明：明确报「本批次没有收录 …」，不套用别的文明或领袖材料；
- 同一文明但规则集不同：同样按缺失处理，并说明材料只覆盖哪些规则集；
- 材料只给能力与触发前提，**不规定固定回合的必然打法**。
...
4. **只推进成功、简报失败**：回执明确写「推进已确认，简报待重取」并带
...
`calls` 是采集期间的 Lua 往返次数（对连接层计数），`chars` 是简报正文长度。
...
- 真实存档下的 `calls` / `elapsed_ms` / `chars`，以及是否需要在保持胜利态势的前提下
  降低慢变字段频率；
