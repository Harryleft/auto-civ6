...
`end_turn` 的等待回执区分已确认结果和诊断状态：

| 返回 | 已知事实 | 后续操作 |
|---|---|---|
| `TURN_PENDING` | 本次观察窗口结束，原请求结果尚未确认 | 可以继续观察；同一在途请求不再次发送 `ACTION_ENDTURN` |
| `HANG` | 从首次提交起的实际经过时间达到诊断阈值 | 停止循环并报告现场证据；这不是引擎死锁的证明，旧请求仍保留 |
| `BRIEF_PENDING:RE_READ_OVERVIEW_ONLY` | 回合推进已确认，后续简报尚未完成 | 只重读 `get_game_overview`，不重复已发动作 |

单次调用期限不超过三分钟。总请求等待用 monotonic 起点计算，包括查询、锁等待与
两次调用之间的间隔；不再把 sleep 参数相加当作真实经过时间。达到阈值只停止等待，
不等于远端取消。议会预计到期但持续 `no_session` 也会到达有界诊断出口，
不再优先返回无上限的 `CONGRESS_NOT_DRIVEN`。投票已确认提交或结果未知后，
驱动器都不自动再次提交；继续处理 pending 议会时不额外发送 `ACTION_ENDTURN`。

同一逻辑回合的提交权被串行保护，在可能发送前就记录在途状态。响应丢失、协程取消、
弹窗关闭、等待超时与 HANG 都不能自动授权重发；只有明确未发送、确认推进或确认
换局才能结束对应状态。这是在单个服务进程内避免未知结果主动重发的保证，
不声称没有游戏端幂等 ID 时可以跨进程崩溃实现绝对 exactly-once。

### 恢复是独立操作

通用工具管道不再因连续连接异常自动杀进程或读档，`end_turn` 也不承担恢复职责。
需要恢复时，由操作者根据进程状态、在途请求和可用存档选择独立的恢复工具。
`load_game_save` 的加载响应未知时不自动转入重启；不支持对局内加载的平台会明确
提示使用 `restart_and_load`。

### 读档与缓存

清缓存只作废旧证据，不能清掉远端可能仍在执行的结束回合请求。存档不存在或索引
不合法等明确拒绝保留原请求。加载已提交但结果未知时，设置 `reload_pending` 暂停写入。

对局内加载前会先在 InGame 安装并确认一个临时标记。`get_game_overview` 在恢复待确认
状态下先只读核验：连接重新握手后，必须读到可玩的局面且旧 Lua 标记已消失，才认定
对局分支已替换并清除旧请求；同种子同回合读档也按此核验。仅重新连接或读取同一回合
不算加载成功。没有标记证据的未知读档继续停写，交由独立恢复处理。菜单加载则以
从 MainMenu 进入可玩 GameCore/InGame 并读到有效回合为核验依据。

### AI 处理期间的发送约束

纯继续观察跳过入口简报、身份、日记和完整预检，游戏结束检查走 GameCore。
后台普通 InGame 查询在取得发送锁后再次检查阶段，排队时阶段已改变也不得发出。
结束回合首次提交、确认所需的外交/议会操作和显式读档分别使用内部的窄例外；
这些例外不是给任意游戏写入开放的通行证。
...
| 场景 | 行为 | agent 处置 |
|---|---|---|
| 变异命令（move/attack/购买/end_turn 等，含 `run_lua` 的 ingame/state-index 通道）发送时连接死亡或超时未收到 sentinel | 抛 `MutationOutcomeUnknownError`（ConnectionError 子类）；命令**至多发送一次，绝不自动重发**（超时同样视为结果未知，非普通失败） | 不要立刻重试同一动作——先 `get_units`/`get_cities` 核实动作是否已实际生效，再决定重试或放弃 |
