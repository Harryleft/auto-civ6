...
任何任务交付前（含中间交付），必须先读取 [CHECKLIST.md](CHECKLIST.md) 并逐项核对：

1. 逐项回答清单问题，不适用项标注 `N/A` 并说明原因。
2. 未通过项必须在交付前处理完毕；确实无法处理的，在交付说明中显式列出。
3. 最终回复末尾必须报告：`清单核对：通过 x/16，N/A y`。
4. 清单是防遗漏的唯一载体；发现清单与本文冲突时，按更严格者执行并向用户报告，待修订清单。
