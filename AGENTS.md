...
1. `get_game_overview`：读取当前回合、规则集、运行策略、治理状态和压缩蛮族摘要。
2. 威胁评估：总览摘要或服务器告警显示存在营地/近城蛮族时，调用 `get_barbarian_overview` 获取完整坐标（营地是刷兵源头，不能只看当前可见的蛮族单位）；与敌方军力检查（见决策硬规则）并列，不固定顺序。
